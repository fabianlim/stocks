...
# function to convert strings to numerics
    ...
    for c in df.columns:
        ...
        try:
            ...
            if isinstance(elem, basestring):
                ...
                print "[OK] column [{}] string type converted sucessfully".format(c)
        except IndexError:
...
def drop_stocks_with_low_volume(df):
    ...
    dropped_stocks = [a for a in df.index.levels[0] if
            grp['pe_ratio'].count().ix[a] == 0 or grp['volume'].mean().ix[a] < 100000]
...
if __name__ == '__main__':
    ...
    #df['last_trade'] = tidy_datetime(df['last_trade_date'], df['last_trade_time'])
